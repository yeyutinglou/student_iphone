//
//  JSPlaybackCoursewareViewController.m
//  kxmovieapp
//
...
NSString * const JsPlaybackCoursewareParameterMinBufferedDuration = @"JsPlaybackCoursewareParameterMinBufferedDuration";
NSString * const JsPlaybackCoursewareParameterMaxBufferedDuration = @"JsPlaybackCoursewareParameterMaxBufferedDuration";
NSString * const JsPlaybackCoursewareParameterDisableDeinterlacing = @"JsPlaybackCoursewareParameterDisableDeinterlacing";
...
@interface JSPlaybackCoursewareViewController () {
    ...
    KxMovieGLView       *_glViewCourseware;
...
@implementation JSPlaybackCoursewareViewController
                             ...
                             audioEnabled:(BOOL)audioEnabled
{
    ...
    JSPlaybackCoursewareViewController *vc = [[JSPlaybackCoursewareViewController alloc] initWithContentPath: path parameters: parameters audioEnabled:audioEnabled];
             ...
             audioEnabled:(BOOL)audioEnabled
{
    ...
    if (self) {
        ...
        __weak JSPlaybackCoursewareViewController *weakSelf = self;
        
        KxMovieDecoder *decoder = [[KxMovieDecoder alloc] init];
        
        decoder.interruptCallback = ^BOOL(){
            
            __strong JSPlaybackCoursewareViewController *strongSelf = weakSelf;
...
- (void)loadView
{
    ...
    _bottomBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, height-50, width, 50)];
    _bottomBar.tintColor = [UIColor blueColor];
...
- (void) handleTap: (UITapGestureRecognizer *) sender
{
    if (sender.state == UIGestureRecognizerStateEnded) {
        ...
        if (sender == _tapGestureRecognizer) {
            ...
            //通知代理隐藏屏幕
            if ([self.delegete respondsToSelector:@selector(hideBottomBar)])
            {
                [self.delegete hideBottomBar];
            }
            ...
            //通知代理切换两个视频
            if ([self.delegete respondsToSelector:@selector(exchangeVideoCourseware:)])
            {
                [self.delegete exchangeVideoCourseware:NO];
            }
               ...
               withError: (NSError *) error
{
    ...
    if (!error && decoder) {
        ...
        if (_parameters.count) {
            ...
            val = [_parameters valueForKey: JsPlaybackCoursewareParameterMinBufferedDuration];
            if ([val isKindOfClass:[NSNumber class]])
                _minBufferedDuration = [val floatValue];
            
            val = [_parameters valueForKey: JsPlaybackCoursewareParameterMaxBufferedDuration];
            if ([val isKindOfClass:[NSNumber class]])
                _maxBufferedDuration = [val floatValue];
            
            val = [_parameters valueForKey: JsPlaybackCoursewareParameterDisableDeinterlacing];
            if ([val isKindOfClass:[NSNumber class]])
...
- (void) setupPresentView
{
    ...
    if (_decoder.validVideo) {
        _glViewCourseware = [[KxMovieGLView alloc] initWithFrame:bounds decoder:_decoder];
    }
    
    if (!_glViewCourseware) {
...
- (UIView *) frameView
{
    return _glViewCourseware ? _glViewCourseware : _imageView;
}
...
- (CGFloat) presentVideoFrame: (KxVideoFrame *) frame
{
    if (_glViewCourseware) {
        ...
        [_glViewCourseware render:frame];
...
- (void) updateHUD
{
    ...
    //通知代理更新---progressLabel数据
    if ([self.delegete respondsToSelector:@selector(updateTopHud:duration:type:)])
    {
        [self.delegete updateTopHud:position duration:duration type:@"courseware"];
    }
