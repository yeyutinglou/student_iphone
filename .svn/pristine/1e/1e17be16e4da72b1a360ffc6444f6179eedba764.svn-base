...
- (void)load{
    ...
    if ([kDownloadType isEqualToString:@"pdf"])
    {
        ...
        title.text = [NSString stringWithFormat:@"%@.%@",_dict[@"name"], @"pdf"];
    }
    else
    {
        pdfUrl = _dict[@"resUrl"];
    ...
    if (self.isVideo) {
        ...
        if ([[BeeFileCache sharedInstance] hasObjectForKey:title.text]) {
            icon.hidden = YES;
            status.hidden = NO;
            status.text = @"已下载";
            if (_isMyClass) {
                [self.contentView makeTappable:MyClassBoard.OPEN_FILE withObject:title.text];
            }
            else {
                [self.contentView makeTappable:self.isCurriculum?CurriculumBoard.OPEN_FILE:ClassDetailBoard.OPEN_FILE withObject:title.text];
            }
        }
        else {
...
ON_SIGNAL2(DownloadPopupCell, signal)
{
    if ([signal is:DownloadPopupCell.DOWNLOAD])
    {
        ...
        [JYDModelLocator shareModelLocator].isDownloading = YES;
...
- (void)handleRequest:(BeeRequest *)request
{
    if(request.failed)
    {
        ...
        [JYDModelLocator shareModelLocator].isDownloading = NO;
    }
    else if (request.succeed)
    {
        ...
        status.text = @"已下载";
        [[BeeFileCache sharedInstance] setObject:request.responseData forKey:title.text];
        [self load];
...
-(void)downloadSoucreCount{
    ...
    JYDLog(@"%@?id=%@&resId=%@", url, kUserId,  _dict[@"resId"]);
