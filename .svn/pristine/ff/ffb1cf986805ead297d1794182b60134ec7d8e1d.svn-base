...
- (void)showPhotoViewAtIndex:(int)index
{
    ...
    for (BeeUIImageView *element in _photoScrollView.subviews) {
        if (element.tag == 9527) {
            [element removeFromSuperview];
        }
    }
    if (photo.strDescription) {
        BeeUIImageView *imgV = [BeeUIImageView spawn];
        imgV.frame = CGRectMake(photoViewFrame.origin.x, _photoScrollView.bottom-50, _photoScrollView.width, 50);
        imgV.userInteractionEnabled = YES;
        //imgV.backgroundColor = [UIColor grayColor];
        imgV.tag = 9527;
        [_photoScrollView addSubview:imgV];
        
        BeeUIImageView *bg = [BeeUIImageView spawn];
        bg.frame = imgV.bounds;
        bg.backgroundColor = [UIColor blackColor];
        bg.alpha = 0.5;
        [imgV addSubview:bg];
        
//        BaseLabel *label = [BaseLabel initLabel:photo.strDescription font:FONT(14) color:[UIColor whiteColor] textAlignment:NSTextAlignmentLeft];
//        label.frame = CGRectMake(10, 0, imgV.width-80, imgV.height);
//        [imgV addSubview:label];
        
        BeeUIButton *btnDelete = [BeeUIButton spawn];
        btnDelete.frame = CGRectMake(imgV.width-80, 0, 60, 50);
        [btnDelete setImage:IMAGESTRING(@"btn_photo_delete") forState:UIControlStateNormal];
        [btnDelete addSignal:MJPhotoBrowser.DELETE forControlEvents:UIControlEventTouchUpInside object:photo.url];
        [imgV addSubview:btnDelete];
    }
...
ON_SIGNAL2( MJPhotoBrowser, signal )
{
    if ([signal is:MJPhotoBrowser.DELETE]) {
        //NSString *strUrl = signal.object;
        //[_delegate performSelector:@selector(deletePhoto:) withObject:strUrl];
    }
}
