...
- (NSMutableArray *)queryTeamMessage:(NSString *)strTeamId{
    NSMutableArray *array = [[NSMutableArray alloc] init];
    NSString *sqlQuery = [NSString stringWithFormat:@"SELECT * FROM %@ WHERE ( %@ = %@ AND %@ = %@ AND %@ = %@) ORDER BY %@",TABLE_NAME,GROUPID,strTeamId,MSGTYPE,@"2",USERID,kUserInfo[@"id"],SENDDATE];
    sqlite3_stmt * statement;
...
- (void)deleteTeamMessage:(NSMutableDictionary *)dictMessage{
    NSString *sqlDelete = [NSString stringWithFormat:@"DELETE FROM %@ WHERE %@ = %@ AND %@ = %@",TABLE_NAME,GROUPID,dictMessage[@"teamId"],USERID,kUserInfo[@"id"]];
    [self execSql:sqlDelete];
}

- (void)deleteFriendMessage:(NSMutableDictionary *)dictMessage{
    NSString *sqlDelete = [NSString stringWithFormat:@"DELETE FROM %@ WHERE (%@ = %@ AND %@ = %@) OR (%@ = %@ AND %@ = %@)",TABLE_NAME,USERID,dictMessage[@"friendId"],SENDUSERID,kUserInfo[@"id"],USERID,kUserInfo[@"id"],SENDUSERID,dictMessage[@"friendId"]];
    [self execSql:sqlDelete];
}
