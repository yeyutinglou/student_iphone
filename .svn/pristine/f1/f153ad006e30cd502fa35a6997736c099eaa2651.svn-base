...
@implementation ResetPasswordBoard
DEF_SIGNAL(SET)
...
ON_SIGNAL2( ResetPasswordBoard, signal )
{
    if ([signal is:ResetPasswordBoard.SET]) {
        if (kStrTrim(field[0].text).length==0) {
            [[BeeUITipsCenter sharedInstance] presentFailureTips:@"请输入新密码"];
            return;
        }
        else if (kStrTrim(field[1].text).length==0) {
            [[BeeUITipsCenter sharedInstance] presentFailureTips:@"请输入确认密码"];
            return;
        }
        else if (![kStrTrim(field[0].text) isEqualToString:kStrTrim(field[1].text)]){
        ...
        BeeHTTPRequest *request = [self POST:[NSString stringWithFormat:@"%@%@",kSchoolUrl,@"app/user/reset_pwd.action"]].PARAM(@"phone",self.strPhone).PARAM(@"validCode",self.strCode).PARAM(@"newPassword",kStrTrim(field[0].text)).PARAM(@"confirmPwd",kStrTrim(field[0].text));
    ...
    else if (request.succeed)
    {
        ...
        switch ([json[@"STATUS"] intValue]) {
            case 0:
            {
                ResetPasswordSuccessBoard *board = [[ResetPasswordSuccessBoard alloc] init];
                [self.stack pushBoard:board animated:YES];
                board.strPassword = field[0].text;
                board.strPhone = _strPhone;
...
- (void)loadContent{
    ...
    imgBg.frame = CGRectMake(25, 20, 271, 80);
    imgBg.image = IMAGESTRING(@"reset_bg");
    ...
    [btnOK addSignal:ResetPasswordBoard.SET forControlEvents:UIControlEventTouchUpInside];
